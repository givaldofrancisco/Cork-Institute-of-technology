...
ggplot(sheet_1, aes(factor(Age_Range), fill = factor(CriminalRecord))) +
  geom_bar(position = position_dodge2(preserve = "single"))

# creating a cross table between range of age and Criminal records
ctable(sheet_1$Age_Range,sheet_1$CriminalRecord)
...
ggplot(sheet_1, aes(x = Weight, y =  Height)) +
...
sheet_1 %>% # data set sheet_1
  arrange(desc(Height)) %>% # Arrange variable Height 
  mutate(CriminalRecord = factor(CriminalRecord)) %>% #turn criminal record in factor and a new variable with mutate function
  ggplot(aes(x=Salary, y=Age, size=Height, color=CriminalRecord)) + #plot graph with variables
...
ggplot(sheet_1, aes(x =  Height, y =Weight , color = CriminalRecord)) +
  geom_point() + #insert the points
  facet_grid(~ Age_Range) # separete the grid on total of Age_range group
...
ggplot(sheet_1, aes(x = Age, y =  Height)) +#determine the axes x and y, and also the color
...
#density graph comparing BMI and CriminalRecord
ggplot(data=sheet_1, aes(x=BMI, group=CriminalRecord, fill=CriminalRecord)) +
...
ggplot(data = FinalDataSet, aes(Age_Range, BMI)) +
...
ggplot(FinalDataSet, aes(x = Age, y = Height, colour = Health, size = Weight)) +
  geom_point()
